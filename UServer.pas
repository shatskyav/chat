...
begin
      ...
      if (FmChat.Tag=2) then
       begin
        if (tcl1.Connected) then ShowMessage('Подключились')
                            else Close;
        //авторизация
        s:='User';
        tcl1.Sendln(s);
        //имя пользователя
        tcl1.Sendln(User);
        // пароль пользователя передаем закодированный
                ...
                if FData='User' then
                 begin
                   ...
                   //проверка пароля
                    ...
                    //если новый пользователь добавляем в список
                    if password='' then
                     begin
